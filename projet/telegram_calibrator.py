...
async def calibrate_channel(client: TelegramClient, channel_info: Dict, config: Dict = None) -> Dict:
    ...
    try:
        ...
        # NOTE: Le vrai winrate (comparaison au prix réel du broker) est calculé
        # séparément via price_reference.calculate_real_winrate(), qui utilise
        # les références de prix/heure capturées par l'utilisateur depuis NT8
        # (indicateur CalibrationPanel + capture d'écran + OCR). Voir Accueil
        # → section "Repères de Prix NinjaTrader".
        result = {
            'status': status,
            'score': round(score, 1),
            'metrics': {
            ...
            # Stocker TOUS les signaux (pas juste un échantillon) pour permettre
            # un backtest winrate sur toute la période récupérée. Plus il y a de
            # signaux, plus le winrate est statistiquement significatif.
            'signals_sample': analysis['signals'] if analysis['signals'] else []
        }
